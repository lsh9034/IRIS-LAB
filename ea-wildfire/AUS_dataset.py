...
import gc
import h5py
class AUS_forestfire(Dataset):
    def __init__(self, X_info_path, X_info_name, Y_path, Y_name, delete_col=[], transform=None, norm='Z'):
        
        self.fix_size=100000
        '''Setting preinformation'''
        self.real_columns = [0,1,2,3,4,6,7,8,9,10,11,12]
        ...
        self.EA_means = [0, 288.7456166679585, 37.37278846096888, 66.93127712119069, 64.7637801858399, 42.99139928717568,
                         4.762841464437443, 0.059951247792144766, 0.05985535932961418]
        
        self.EA_stds = [0.1, 11.345074383967875, 18.66352533934532, 33.78579003929686, 10.160512089023387, 
                        10.231124117777917, 4.811568768671124, 1.7408100784129035, 1.6806614843709322]
        ...
        filtered_data_info = np.delete(self.pre_data_info, [0,1,2,3,8,9], -1) #info 데이터에서 날짜 데이터만 사용
        ...
        #set dummy data
        self.data=[]
        self.nan_idx=np.array([], dtype=np.int32)
        
        '''Handling label data'''
        mat_file = scipy.io.loadmat(Y_path)
        self.label = torch.tensor(mat_file[Y_name].flatten(), dtype=torch.float32)
    
    def load_subset(self, path, subset_name, subset_id):
        del self.data
        mat_file = scipy.io.loadmat(path)
        self.data = mat_file[subset_name]
        
        nan_idx = np.where(np.isnan(self.data))[0]
        self.data[nan_idx]=0
        nan_idx = np.array(list(set(nan_idx)), dtype=np.int32)
        nan_idx.sort()
        nan_idx += self.fix_size*(subset_id-1)
        self.nan_idx = np.append(self.nan_idx, nan_idx)

        #self.normalize(self.data)
        self.data = np.delete(self.data, self.delete_col, -1) #Cloud 컬럼 정보 사용 안함
    ...
    def __getitem__(self, index):
        ...
        abs_idx = self.fix_size*(self.subset_id-1) + index
        return self.data_info[abs_idx], data, self.label[abs_idx]
    
    
filenames = [0]+['dataset_AUS_X_tr_CNN_'+str(i) for i in range(1,16)]
AUS_PATH = '/share/wildfire-2/Wildfire_Detection/East_Asia_revision2/dataset/processed/processed/'

def load_info_and_label(delete_col):
    aus_valset = AUS_forestfire(AUS_PATH+'dataset_AUS_info_tr_CNN',
                    'info_tr',
                    AUS_PATH+'dataset_AUS_Y_tr_CNN',
                    'Y_tr',
                    delete_col=delete_col)
    return aus_valset

def load_AUS_DataLoader(aus_dataset, subset_id):
    aus_dataset.load_subset(AUS_PATH + filenames[subset_id], 'X_tr_half1',subset_id)
    aus_dataloader = DataLoader(aus_dataset, batch_size=512, shuffle=False, num_workers=2)
    print(f'Complete AUS Subset {subset_id} Data Loading')
    return aus_dataset, aus_dataloader

def get_label_by_subset(aus_dataset:AUS_forestfire):
    subset_id = aus_dataset.subset_id
    start_idx = aus_dataset.fix_size*(aus_dataset.subset_id-1)
    end_idx = start_idx + len(aus_dataset.data)
    return aus_dataset.label[start_idx:end_idx]



class AUS_forestfire_full(Dataset):
    def __init__(self, X_info_path, X_info_name, Y_path, Y_name, delete_col=[], transform=None, norm='Z', mode='Full'):
        ...
        self.pre_data_info = mat_file[X_info_name]
        filtered_data_info = np.delete(self.pre_data_info, [0,1,2,3,8,9], -1) #info 데이터에서 날짜 데이터만 사용
        self.data_info = torch.tensor(filtered_data_info, dtype=torch.float32)
        del filtered_data_info
        '''Handling AUS Data'''
        if mode=='Full':
    ...
    def clip_one_col(self, data, col, mi, ma):
...
def load_AUS_FULL_DataLoader(delete_col, norm='Z', mode='Full'):
    aus_dataset = AUS_forestfire_full(
        AUS_PATH+'dataset_AUS_info_tr_CNN',
        'info_tr',
        AUS_PATH+'dataset_AUS_Y_tr_CNN',
        'Y_tr',
        delete_col=delete_col,
    ...
    aus_dataloader = DataLoader(aus_dataset, batch_size=512, shuffle=False, num_workers=2)
    print(f'Complete AUS FULL Data Loading')
    return aus_dataset, aus_dataloader




class AUS_forestfire_full2(Dataset):
...
def load_AUS_FULL2_DataLoader(delete_col, norm='Z', mode='Full'):
    aus_dataset = AUS_forestfire_full2(
        AUS_PATH+'dataset_AUS_Y_tr_CNN',
        'Y_tr',
        delete_col=delete_col,
        norm=norm,
