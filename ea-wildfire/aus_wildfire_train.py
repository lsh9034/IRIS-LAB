...
def train_resnet(band_size, path=None):
    ...
    train.train_loop(model=resnet18, epochs=40, train_dataloader=train_dataloader, valid_dataloader=val_dataloader,
                     ...
                     model_name='AUS_ResNet18-'+str(band_size)+'bands_znorm_ver2_1', save_best=True)#, scheduler=resnet_manager.scheduler)
    print('end train')
...
trainset, _ = AUS_dataset.load_AUS_FULL2_DataLoader(delete_col=delete_col, norm='Z', mode='Filter')
trainset, valset = torch.utils.data.random_split(trainset, [0.95, 0.05])
train_dataloader = DataLoader(trainset, batch_size=512, shuffle=True, num_workers=2)
val_dataloader = DataLoader(valset, batch_size=512, shuffle=True, num_workers=2)
