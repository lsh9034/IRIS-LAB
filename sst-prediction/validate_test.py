...
sst_valid_dataset = dataset_sst.SSTDataset(3, predict_day=7 ,img_interval=1, mode='test')
data, label = sst_valid_dataset[0]
T, C, H, W = label.shape
print(T,C,H,W)
record = torch.load('./models/test_best.pkl')
...
f(label, f'val_label_{predict_mode}_{example_idx}', mi, ma)
f(total_pred, f'val_pred_{predict_mode}_{example_idx}', mi, ma)
